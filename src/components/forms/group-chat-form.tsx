...
import { sendGroupMessageAction } from "@/app/actions/social";
...
export function GroupChatForm({ groupId }: { groupId: string }) {
  const [state, formAction] = useActionState(sendGroupMessageAction, initialState);
  const [body, setBody] = useState("");
  const { send } = useRealtimeTopic(useMemo(() => [`group:${groupId}`], [groupId]));
      ...
      <div className="flex items-end gap-3">
        <textarea
          ...
          onChange={(event) => {
            ...
            setBody(nextValue);
            if (nextValue.trim()) {
              send({ type: "typing", topic: `group:${groupId}` });
            }
          }}
        />
